...
static inline void write_script_implementations(ofstream& wto, EnigmaStruct *es, int mode) {
  ...
  for (int i = 0; i < es->scriptCount; i++) {
    ...
    wto << ")\n{\n";
    if (mode == emode_debug) {
      wto << "  enigma::debug_scope enigma_current_debug_scope(\"script '" << es->scripts[i].name << "'\");\n";
    }
    wto << "  ";
...
static inline void write_event_func(ofstream& wto, const parsed_event &event, string objname, string evname, int mode) {
  ...
  if (mode == emode_debug) {
    wto << "  enigma::debug_scope enigma_current_debug_scope(\"event '" << evname << "' for object '" << objname << "'\");\n";
  }
  wto << "  ";
